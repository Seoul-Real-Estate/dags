...
@task
def get_apartment_rent(reclass, url, key, columns):
    raw_df = reclass.data_extract(url, key, columns, reclass.apartment_rent_list)
    return raw_df
...
def check_new(new_df):
    if len(new_df) > 0:
        return "apartment_trade_uploadS3"
    else:
        return "end_task"
...
with DAG(
    ...
    raw_columns = ['년', '월', '일', '법정동', '아파트',  '전용면적', '갱신요구권사용', '건축년도', '지번', '계약구분', '계약기간', '보증금액', 
    ...
    raw_df = get_apartment_rent(sre, url, key, raw_columns)
    ...
    before_fname1 = f"apartment_rent_{mon1}_{yesterday}.csv"
    now_fname1 = f"apartment_rent_{mon1}_{today}.csv"
    upload_fname1 = f"apartment_rent_upload_{mon1}_{today}.csv"

    raw_df1 = get_apartment_rent(sre1, url, key, raw_columns)
    preproc_df1 = apartment_rent_preproc(sre1, raw_df1, preproc_cols)
    upload_task1 = apartment_rent_uploadS3(sre1, preproc_df1) 
    new_df1 = apartment_rent_compare(sre1, before_fname1, now_fname1, fin_cols)
    ...
    load_task1 = apartment_rent_new_uploadS3(new_df1, upload_fname1)
    load_redshif_task1 = apartment_rent_load(schema, table, upload_fname1, credential)
    ...
    before_fname2 = f"apartment_rent_{mon2}_{yesterday}.csv"
    now_fname2 = f"apartment_rent_{mon2}_{today}.csv"
    upload_fname2 = f"apartment_rent_upload_{mon2}_{today}.csv"

    raw_df2 = get_apartment_rent(sre2, url, key, raw_columns)
    preproc_df2 = apartment_rent_preproc(sre2, raw_df2, preproc_cols)
    upload_task2 = apartment_rent_uploadS3(sre2, preproc_df2) 
    new_df2 = apartment_rent_compare(sre2, before_fname2, now_fname2, fin_cols)
    ...
    load_task2 = apartment_rent_new_uploadS3(new_df2, upload_fname2)
    load_redshif_task2 = apartment_rent_load(schema, table, upload_fname2, credential)
