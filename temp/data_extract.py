...
startdate = '202404'
dates = [datetime.strftime(datetime.strptime(startdate, "%Y%m") - relativedelta(months= i), "%Y%m") for i in range(16)]
...
data_extract(url, key, dates, cols, "아파트전월세", apartment_rent)
...
def billa_rent(data, source, cols):
    for row in source:
        ...
        data.append([year, month, day, dong, name, area, uses, buildyear, addr, type, length, deposit, rent, before_deposit, before_rent, code, floor])
data_extract(url, key, dates, cols, "연립다세대전월세", billa_rent)
...
#         except: buildingnum = " "
#         data.append([year, month, day, dong, name, area, price, buildyear, addr, code, floor, yn, yndate, type, realtor, contract_date, seller, buyer, buildingnum])
# data_extract(url, key, dates, cols, "연립다세대매매", billa_sell)