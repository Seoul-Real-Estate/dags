...
@task
def get_apartment_trade(reclass, url, key, origincols, columns):
    raw_df = reclass.data_extract(url, key, origincols, columns, reclass.apartment_trade_list)
    return raw_df

@task
def apartment_trade_preproc(reclass, raw_df, columns):
    preproc_df = reclass.apartment_trade_preprocessing(raw_df, columns)
    return preproc_df

@task
def apartment_trade_uploadS3(reclass, preproc_df):
    csv_buffer = StringIO()
    ...
    s3_hook.load_string(
        ...
        key=f'data/apartment_trade_{reclass.trademonth}_{reclass.rundate}.csv',
...
def check_date(today):
    if today[-2:] == '01':
        return "apartment_trade_load_1st"
    else:
        return "apartment_trade_compare"
    
@task
def apartment_trade_load_1st(reclass, preproc_df, columns, schema, table, upload_fname, credentials):
    logging.info("apartment trade load 1st started")
    ...
    logging.info("apartment trade load to redshift")
...
@task
def apartment_trade_compare(reclass, before_fname, now_fname, columns):
    s3_hook = S3Hook(aws_conn_id='s3_conn') 
    bucket_name = 'team-ariel-2-data'
...
@task
def apartment_trade_load(schema, table, upload_fname, credentials):
    logging.info("apartment trade load started")
    cur = get_Redshift_connection()
    try:
        cur.execute("BEGIN;")
        sql = f"""
...
with DAG(
    dag_id='trade_apartment',
    start_date=datetime(2024, 7, 24),
    timezone='Asia/Seoul',  # 한국 표준시(KST) 설정
    schedule='55 10 * * *',  # 매일 09:50 AM KST에 실행
    max_active_runs=1,
    catchup=False, 
    default_args={
        'retries': 0,
    ...
    apartment_trade_cols = ['year', 'month', 'day', '법정동명', '건물명', '전용면적', '물건금액', '건축년도', '지번', '지역코드', '층', '취소일', '신고구분', '등기일자']
    ...
    before_fname = f"apartment_trade_{today[:6]}_{yesterday}.csv"
    now_fname = f"apartment_trade_{today[:6]}_{today}.csv"
    upload_fname = f"apartment_trade_upload_{today[:6]}_{today}.csv"

    raw_df = get_apartment_trade(sre, url, key, apartment_trade_origin_cols, apartment_trade_cols)
    preproc_df = apartment_trade_preproc(sre, raw_df, preproc_cols)
    upload_task = apartment_trade_uploadS3(sre, preproc_df) 
    check_date_task = BranchPythonOperator(
        task_id = "check_date",
        python_callable=check_date,
        op_args=[today],
    )
    load_1st_task = apartment_trade_load_1st(sre, preproc_df, fin_cols, schema, table, upload_fname, credential)
    new_df = apartment_trade_compare(sre, before_fname, now_fname, fin_cols)
    ...
    load_task = apartment_trade_new_uploadS3(new_df, upload_fname)
    load_redshif_task = apartment_trade_load(schema, table, upload_fname, credential)
