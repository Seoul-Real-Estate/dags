...
with DAG(
    dag_id='trade_billa_1mon',
    start_date=datetime(2024, 7, 24),
    timezone='Asia/Seoul',  # 한국 표준시(KST) 설정
    schedule='15 11 * * *',  # 매일 09:50 AM KST에 실행
    max_active_runs=1,
    catchup=False, 
    default_args={
        'retries': 0,
    ...
    raw_df = get_billa_trade(sre1, url, key, billa_trade_origin_cols, billa_trade_cols)
    preproc_df = billa_trade_preproc(sre1, raw_df, preproc_cols)
    upload_task = billa_trade_uploadS3(sre1, preproc_df) 
    new_df = billa_trade_compare(sre1, before_fname1, now_fname1, fin_cols)
    ...
    load_task = billa_trade_new_uploadS3(new_df, upload_fname1)
    load_redshif_task = billa_trade_load(schema, table, upload_fname1, credential)
