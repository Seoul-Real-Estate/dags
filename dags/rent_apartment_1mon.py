...
with DAG(
    dag_id='rent_apartment_1mon',
    start_date=datetime(2024, 7, 24),
    timezone='Asia/Seoul',  # 한국 표준시(KST) 설정
    schedule='0 10 * * *',  # 매일 09:50 AM KST에 실행
    max_active_runs=1,
    catchup=False, 
    default_args={
        'retries': 0,
    ...
    raw_df = get_apartment_rent(sre1, url, key, apartment_rent_origin_cols, apartment_rent_cols)
    preproc_df = apartment_rent_preproc(sre1, raw_df, preproc_cols)
    upload_task = apartment_rent_uploadS3(sre1, preproc_df) 
    new_df = apartment_rent_compare(sre1, before_fname1, now_fname1, fin_cols)
    ...
    load_task = apartment_rent_new_uploadS3(new_df, upload_fname1)
    load_redshif_task = apartment_rent_load(schema, table, upload_fname1, credential)
