...
def check_date(today):
    if today[-2:] == '01':
        return "house_rent_load_1st"
    else:
        return "house_rent_compare"
    
@task
def house_rent_load_1st(reclass, preproc_df, columns, schema, table, upload_fname, credentials):
    logging.info("house rent load 1st started")
    ...
    logging.info("house rent load to redshift")
...
with DAG(
    ...
    schedule='25 10 * * *',  # 매일 09:50 AM KST에 실행
    max_active_runs=1,
    catchup=False, 
    default_args={
        'retries': 0,
    ...
    before_fname = f"house_rent_{today[:6]}_{yesterday}.csv"
    now_fname = f"house_rent_{today[:6]}_{today}.csv"
    upload_fname = f"house_rent_upload_{today[:6]}_{today}.csv"

    raw_df = get_house_rent(sre, url, key, house_rent_origin_cols, house_rent_cols)
    preproc_df = house_rent_preproc(sre, raw_df, preproc_cols)
    upload_task = house_rent_uploadS3(sre, preproc_df) 
    check_date_task = BranchPythonOperator(
        task_id = "check_date",
        python_callable=check_date,
        op_args=[today],
    )
    load_1st_task = house_rent_load_1st(sre, preproc_df, fin_cols, schema, table, upload_fname, credential)
    new_df = house_rent_compare(sre, before_fname, now_fname, fin_cols)
    ...
    load_task = house_rent_new_uploadS3(new_df, upload_fname)
    load_redshif_task = house_rent_load(schema, table, upload_fname, credential)
