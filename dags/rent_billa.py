...
@task
def get_billa_rent(reclass, url, key, origincols, columns):
    raw_df = reclass.data_extract(url, key, origincols, columns, reclass.billa_rent_list)
    return raw_df
...
def check_date(today):
    if today[-2:] == '01':
        return "billa_rent_load_1st"
    else:
        return "billa_rent_compare"
    
@task
def billa_rent_load_1st(reclass, preproc_df, columns, schema, table, upload_fname, credentials):
    logging.info("billa rent load 1st started")
    ...
    logging.info("billa rent load to redshift")
...
def check_new(new_df):
    if len(new_df) > 0:
        return "billa_rent_new_uploadS3"
    else:
        return "end_task"
...
with DAG(
    ...
    billa_rent_origin_cols = ['dealYear','dealMonth', 'dealDay', 'umdNm', 'mhouseNm', 'excluUseAr', 'useRRRight', 'buildYear', 'jibun', 'contractType', 
                    'contractTerm', 'deposit', 'monthlyRent', 'preDeposit', 'preMonthlyRent', 'sggCd', 'floor']
    billa_rent_cols = ['year', 'month', 'day', '법정동명', '건물명', '임대면적', '계약갱신권사용여부', '건축년도', '지번', '신규갱신여부',
    ...
    raw_df = get_billa_rent(sre, url, key, billa_rent_origin_cols, billa_rent_cols)
    ...
    load_1st_task = billa_rent_load_1st(sre, preproc_df, fin_cols, schema, table, upload_fname, credential)
