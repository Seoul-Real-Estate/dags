...
with DAG(
    ...
    before_fname1 = f"house_rent_{mon1}_{yesterday}.csv"
    now_fname1 = f"house_rent_{mon1}_{today}.csv"
    upload_fname1 = f"house_rent_upload_{mon1}_{today}.csv"

    raw_df = get_house_rent(sre1, url, key, house_rent_origin_cols, house_rent_cols)
    preproc_df = house_rent_preproc(sre1, raw_df, preproc_cols)
    upload_task = house_rent_uploadS3(sre1, preproc_df) 
    new_df = house_rent_compare(sre1, before_fname1, now_fname1, fin_cols)
    ...
    load_task = house_rent_new_uploadS3(new_df, upload_fname1)
    load_redshif_task = house_rent_load(schema, table, upload_fname1, credential)
