...
FILE_NAME = "cinema_near_estate.csv"
...
#  테이블 생성 쿼리
CREATE_QUERY = """
CREATE TABLE IF NOT EXISTS raw_data.cinema_near_estate (
    estate_id VARCHAR(500),
    x FLOAT,
    y FLOAT,
    type VARCHAR(500),
    cinema_name_1 VARCHAR(500),
    longitude_1 FLOAT, 
    latitude_1 FLOAT,
    cinema_name_2 VARCHAR(500),
    longitude_2 FLOAT, 
    latitude_2 FLOAT,
    cinema_name_3 VARCHAR(500),
    longitude_3 FLOAT, 
    latitude_3 FLOAT
);
"""
...
def getDataCount(**context):
    ...
    sql = """
    SELECT COUNT(*) FROM raw_data.cinema_near_estate;
    """
...
def cinema_extract(**context):
   redshift_hook = RedshiftSQLHook(redshift_conn_id='rs_conn')
    
   sql = """
    SELECT screen_name, longitude, latitude
    FROM analytics.seoul_cinema
   ...
   context["ti"].xcom_push(key="cinema_extracted_data", value=rows)


def cinema_transform_1(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    for index_1, row_1 in address_df.iterrows():
        ...
        for i in range(len(cinema_list)):
            ...
            cinema_data.append(cinema_list[i][2])
        cinema_transformed_data.append(cinema_data)

    context["ti"].xcom_push(key="cinema_transformed_data_1", value=cinema_transformed_data)

def cinema_transform_2(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    context["ti"].xcom_push(key="cinema_transformed_data_2", value=cinema_transformed_data)


def cinema_transform_3(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    context["ti"].xcom_push(key="cinema_transformed_data_3", value=cinema_transformed_data)


def cinema_transform_4(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    context["ti"].xcom_push(key="cinema_transformed_data_4", value=cinema_transformed_data)


def cinema_transform_5(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    context["ti"].xcom_push(key="cinema_transformed_data_5", value=cinema_transformed_data)


def cinema_transform_6(**context):
    cinema_extracted_list = context["ti"].xcom_pull(key="cinema_extracted_data")
    ...
    context["ti"].xcom_push(key="cinema_transformed_data_6", value=cinema_transformed_data)


def combineAllData(**context):
    transformed_data_1 = context["ti"].xcom_pull(key="cinema_transformed_data_1")
    transformed_data_2 = context["ti"].xcom_pull(key="cinema_transformed_data_2")
    transformed_data_3 = context["ti"].xcom_pull(key="cinema_transformed_data_3")
    transformed_data_4 = context["ti"].xcom_pull(key="cinema_transformed_data_4")
    transformed_data_5 = context["ti"].xcom_pull(key="cinema_transformed_data_5")
    transformed_data_6 = context["ti"].xcom_pull(key="cinema_transformed_data_6")
...
def loadToRedshift(autocommit=True):
   ...
   cursor = conn.cursor()
    ...
    # Redshift용 COPY 명령문
   copy_query = f"""
   COPY raw_data.cinema_near_estate
...
CreatecinemaTable = PostgresOperator(
    task_id = "create_cinema_table",
...
cinema_extract_task = PythonOperator(
    task_id = "cinema_extract",
    python_callable=cinema_extract,
    dag=dag
)

cinema_transform_task_1 = PythonOperator(
    task_id = "cinema_transform_1",
    python_callable=cinema_transform_1,
    dag=dag
)

cinema_transform_task_2 = PythonOperator(
    task_id = "cinema_transform_2",
    python_callable=cinema_transform_2,
    dag=dag
)

cinema_transform_task_3 = PythonOperator(
    task_id = "cinema_transform_3",
    python_callable=cinema_transform_3,
    dag=dag
)

cinema_transform_task_4 = PythonOperator(
    task_id = "cinema_transform_4",
    python_callable=cinema_transform_4,
    dag=dag
)

cinema_transform_task_5 = PythonOperator(
    task_id = "cinema_transform_5",
    python_callable=cinema_transform_5,
    dag=dag
)

cinema_transform_task_6 = PythonOperator(
    task_id = "cinema_transform_6",
    python_callable=cinema_transform_6,
    dag=dag
)
