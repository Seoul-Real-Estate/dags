...
with DAG(
    dag_id='trade_apartment_2mon',
    start_date=datetime(2024, 7, 24),
    timezone='Asia/Seoul',  # 한국 표준시(KST) 설정
    schedule='5 11 * * *',  # 매일 09:50 AM KST에 실행
    max_active_runs=1,
    catchup=False, 
    default_args={
        'retries': 0,
    ...
    before_fname2 = f"apartment_trade_{mon2}_{yesterday}.csv"
    now_fname2 = f"apartment_trade_{mon2}_{today}.csv"
    upload_fname2 = f"apartment_trade_upload_{mon2}_{today}.csv"

    raw_df2 = get_apartment_trade(sre2, url, key, apartment_trade_origin_cols, apartment_trade_cols)
    preproc_df2 = apartment_trade_preproc(sre2, raw_df2, preproc_cols)
    upload_task2 = apartment_trade_uploadS3(sre2, preproc_df2) 
    new_df2 = apartment_trade_compare(sre2, before_fname2, now_fname2, fin_cols)
    ...
    load_task2 = apartment_trade_new_uploadS3(new_df2, upload_fname2)
    load_redshif_task2 = apartment_trade_load(schema, table, upload_fname2, credential)
